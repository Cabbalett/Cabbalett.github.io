...
https://arxiv.org/abs/1708.02002 의 Focal loss는 RetinaNet에서 처음 제시된 metric으로, 기존의 one-stage detecetor의 단점인 낮은 성능을 보완할 수 있었다. One-stage detector는 속도는 빠르지만, 그만큼 anchor box를 많이 생성해 극심한 foreground-background class imbalance가 존재했다. 이를 해결하기 위해, 기존의 cross entropy loss에서 well-classified sample들은 작은 가중치를, 그리고 hard sample들에 대해 큰 가중치를 가해 과도한 양의 negative sample들로 인해 학습이 방해되지 않도록 했다.
...
다음 코드는 https://github.com/AdeelH/pytorch-multi-class-focal-loss 를 참조했다.

$x$는 model을 통과하여 나온 $(batch\_size,class,w,h)$의 tensor, $y$는 $(batch\_size,w,h)$의 ground truth tensor이다. 예제로 tensor를 랜덤하게 생성하면,
