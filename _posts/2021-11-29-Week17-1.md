...
$(x,y)$는 train input과 target, $W$는 trainable weight를 나타낸다. 전체 loss의 첫번째 항은 우리가 흔하게 사용하는 training loss이다. 이 논문에서는 $g(\cdot)$함수를 사용해 scaling factor $\gamma$에 penalty를 가해 중요도가 낮은 channel에 해당하는 gamma를 최소화하는 방향으로 학습이 진행되게끔 유도했다. 논문에서는 $g(s)=\left|s\right|$, 즉 L1 loss를 사용했다.
